...
import { kMainColor } from "../../theme/constant";
...
const ThankyouScreen = () => {
  ...
  return (
    ...
    >
      <Typography
        variant="h3"
        style={{ color: kMainColor, textAlign: "center" }}
      >
...
const CartView = () => {
  ...
  const onOrder = () => {
    setIsOrdered(true);
    DbServices.finishCart(user.googleId)
      .then((data) => {
        //
        setCart([]);
      })
          ...
          <Box>
            <Card>
              <CardContent className={classes.container}>
                {cart.length > 0 &&
                  cart.map((item) => {
                    return <CartItemView key={item.item_id} item={item} />;
                  })}
              </CardContent>
            </Card>
